...
function PureMultimodalInput({
    ...
    sendMessage({
      ...
      parts: [
        ...attachments.map((attachment) => ({
          ...
          mediaType: attachment.contentType || attachment.type || "application/octet-stream",
        })),
        {
          type: "text",
          text: input,
  ...
  const uploadFile = useCallback(async (file: File): Promise<Attachment | undefined> => {
    try {
      const result = await uploadDocument(file, 'semantic');

      if (result.success) {
        toast.success(`Uploaded "${result.file_name}" successfully!`);
        ...
        return {
          ...
          url: `rag-doc://${result.document_id}`,
...
export const MultimodalInput = memo(
  ...
  (prevProps, nextProps) => {
    ...
    if (prevProps.onUploadComplete !== nextProps.onUploadComplete) {
      return false;
    }
